...
const sendMessage = () => {
	...
	if (inputChatting.value.trim().length == 0 && !imageInput.files[0]) {
		alert("채팅을 입력하거나 이미지를 선택해주세요.");
		inputChatting.value = "";
		return;
	} else {
		var obj = {
		...
		// 이미지 파일이 있는 경우 업로드 후 URL을 메세지에 추가
		if(imageInput.files[0]) {
			uploadImage(imageInput.files[0])
			.then(image => {
				obj.messageContent += `<br><img src="${image}" alt="Image">`;
...
// 	document.querySelector(".display-chatting").appendChild(messageElement);
// };
